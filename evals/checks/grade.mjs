...
if (name === 'no-trigger') {
...
} else if (name === 'deep-state') {
  const runDir = join(work, '.taskforge', 'inventory-cleanup');
  const required = ['context.md', 'glossary.md', 'decisions.md', 'contract.md', 'state.json', 'iterations.md', 'completion.md'];
  if (!existsSync(runDir)) failures.push('expected .taskforge/inventory-cleanup was not created');
  else {
    for (const file of required) if (!existsSync(join(runDir, file))) failures.push(`missing durable file: ${file}`);
    ...
    if (existsSync(statePath)) {
      ...
      if (state.mode !== 'DEEP') failures.push(`expected DEEP, got ${state.mode}`);
  ...
  if (!existsSync(statePath)) failures.push('durable no-progress state was not created');
  else {
    const state = JSON.parse(readFileSync(statePath, 'utf8'));
...
} else failures.push(`unknown grader case: ${name}`);

console.log(JSON.stringify({ pass: failures.length === 0, score: failures.length === 0 ? 1 : 0, evidence: failures.length === 0 ? [`${name} passed`] : failures }));
